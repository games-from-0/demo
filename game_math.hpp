...
{
    real32_t length = sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
...
{
    real32_t length = sqrt(v.x * v.x + v.y * v.y + v.z * v.z + v.w * v.w);
...
inline matrix4_t perspective(real32_t fov, real32_t aspect_ratio, real32_t near, real32_t far)
{
    ...
    real32_t tan_half = tan(fov / 2.0f);
...
inline matrix4_t m4_rotate(real32_t x_rad, real32_t y_rad, real32_t z_rad)
{
    ...
    if (x_rad != 0)
    {
        r_x.col[1].v[1] = cos(x_rad);
        r_x.col[2].v[1] = -sin(x_rad);

        r_x.col[1].v[2] = sin(x_rad);
        r_x.col[2].v[2] = cos(x_rad);
    }
    if (y_rad != 0)
    {
        r_y.col[0].v[0] = cos(y_rad);
        r_y.col[0].v[2] = -sin(y_rad);

        r_y.col[2].v[0] = sin(y_rad);
        r_y.col[2].v[2] = cos(y_rad);
    }
    if (z_rad != 0)
    {
        r_z.col[0].v[0] = cos(z_rad);
        r_z.col[0].v[1] = -sin(z_rad);

        r_z.col[1].v[0] = sin(z_rad);
        r_z.col[1].v[1] = cos(z_rad);
    }
